...
class Blockchain {
     ...
     */
     _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (block.height>0){
                block.previousBlockHash = self.chain[block.height - 1] ? self.chain[block.height - 1].hash : null;
            }
    ...
    getStarsByWalletAddress (address) {
        ...
        this.validateChain().then(
            errors => typeof errors === 'string' ?  console.log('[SUCCESS] ', errors) : errors.forEach(error => console.log('[ERROR] ', error))
        );
        return new Promise(async (resolve, reject) => {
            let ownedBlocks = self.chain.filter(b => b.owner === address);
            if (ownedBlocks.length === 0) reject(new Error('Address could not be found.'));
            stars = ownedBlocks.map(block => JSON.parse(hex2ascii(block.body)));
            stars ? resolve(stars) : reject(new Error('Stars could not be resolved.'));
        });
    }
                ...
                if (await block.validate()) {
                    if (block.height > 0) { 
                        ...
                        if (block.previousBlockHash !== prevBlock.hash) {
                            errorLog.push(new Error(`Block Heigh ${block.height}:Previous Hash doesn't match.`));
                        }
                    }
                } else {
                    errorLog.push(new Error(`Block #${block.hash}: Not valid`))
                }
            }
